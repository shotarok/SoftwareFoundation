...
Definition filter_even_gt7 (l : list nat) : list nat :=
  (* FILL IN HERE *)
  filter (fun n => negb (ble_nat n 7))(filter evenb l).
...
Theorem snoc_map_rev : forall (X Y :Type) (f : X -> Y) (l : list X) (n : X),
  snoc (map f (rev l)) (f n) = rev ( (f n) :: (map f  l)).
Proof.  intros X Y f l n. simpl. Admitted.
...
Proof.
  ...
  unfold override. rewrite -> H1. apply H0.
Qed.
(** [] *)
...
Proof.
  ...
  SCase "n = S n". intro contra. inversion contra.
  Case "m = S m'.". destruct n as [| n'].
  SCase "n = 0.". intro contra. inversion contra.
  SCase "n = S n'.". simpl. intro H. SearchAbout eq.
...
Proof.
  intros n. induction n as [| n'].
  (* FILL IN HERE *)
  Case "n = 0.". intro m. destruct m as [| m'].
  reflexivity. simpl. reflexivity.
  Case "n = S n'.". intro m. destruct m as [| m'].
...
Proof.
  intros n. induction n as [| n'].
    ...
    (* FILL IN HERE *)
  Case "n = 0.". SearchAbout plus.
  destruct m as [| m']. reflexivity. intro contra. inversion contra.
  Case "n = S n'.". Admitted.
(** [] *)
...
Proof.
  ...
  (* FILL IN HERE *) SearchAbout combine.
  Case "l is [].". intros l1 l2 H. inversion H. reflexivity.
  Case "l is (x, y) :: l'.". intros l1 l2 H. inversion H.
  simpl. Admitted.
(** [] *)
...
Proof.
  ...
  unfold override. remember (beq_nat k1 k2) as k. destruct k.
  apply beq_nat_eq in Heqk. rewrite -> Heqk in H. symmetry. apply H.
  reflexivity.
Qed.
  (** [] *)
...
n    instead write: apply trans_eq with [c,d]. *)
...
Proof.
  (* FILL IN HERE *)
  intros n m o p. remember (minustwo o) as q.
  intros H1 H2. rewrite -> H1 in H2. apply H2.
Qed.
...
Proof.
  (* FILL IN HERE *)
  intros n m p H1 H2. apply beq_nat_eq in H1. apply beq_nat_eq in H2.
  rewrite -> H1. rewrite <- H2. apply beq_nat_refl.
Qed.
...
(* FILL IN HERE *)
  intros X l. unfold fold_length. induction l as [|n l'].
  Case "l is [].". reflexivity.
  Case "l is n :: l'.". simpl.  apply eq_remove_S. apply IHl'.
Qed.  
(** [] *)
