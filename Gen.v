...
Proof.
  ...
  generalize dependent n.
  induction m as [| m'].
  Case "m is 0.". intros n H. destruct n as [|n'].
    SCase "n = 0.". reflexivity. inversion H.
  Case "m is S m'.". intros n H. destruct n as [|n'].
    SCase "n = 0.". inversion H.
    SCase "n is S n'.".  apply eq_remove_S. apply IHm'.
...
Proof.
  ...
  generalize dependent n.
  induction l as [| l'].
  Case "l is [].". intros n H. destruct n as [| n'].
    SCase "n = 0.". reflexivity.
    SCase "n is S n'.". inversion H.
  Case "l is cons.". intros n H. simpl. destruct n as [| n'].
    SCase "n = 0.". inversion H.
...
Theorem index_after_last_informal : forall (X : Type) (l : list X) (n :nat),
                                           
       length l = n -> index (S n) l = None.
Proof.
  intros X. induction l as [| x' l'].
  Case "l is [].". intros n H. simpl. reflexivity.
  Case "l is cons.". intros n H.
  simpl. destruct n as [| n'].
    SCase "n is O.". inversion H.
    SCase "n is S n'.". apply IHl'. simpl in H.
...
Proof.
  ...
  generalize dependent n.
  induction l as [| l'].
  Case "l is [].". destruct n as [| n'].
    reflexivity. intro H. inversion H.
  Case "l is cons.". destruct n as [|n'].
    SCase "n is 0.". intro H. inversion H.
    SCase "n is S n'.". intro H. simpl. apply eq_remove_S.
...
Theorem app_length_cons_lemma : forall (X : Type) (x : X) (l : list X),
   length(l ++ [x]) = S (length (l ++ [])).
Proof.
intros X x l. induction l as [| l'].
Case "l is [].". reflexivity.
Case "l is cons.". simpl. apply eq_remove_S. apply IHl.
Qed.
...
Proof.
  ...
  Case "l1 is cons.".
    ...
    SCase "l2 is [].".
      ...
      rewrite <- eq. apply eq_remove_S.
      symmetry. apply app_length_cons_lemma.
    SCase "l2 is cons.".
    intros n eq. simpl. simpl in eq. destruct n as [|n'].
      SSCase "n is O.". inversion eq.
      SSCase "n is S n'.". SearchAbout (S _ = S _ -> _ = _). apply eq_add_S in eq.
      apply IHl1' in eq. apply eq_remove_S. apply eq.
Qed.     
...
Theorem app_length_repl : forall (X : Type) (l1 l2 : list X),
...
Proof.
  ...
  Case "l1 is cons.".
    induction l2 as [| x2' l2'].
    SCase "l2 is [].". simpl. rewrite -> app_nil. reflexivity.
    SCase "l2. cons.". simpl. rewrite <- IHl2'. apply eq_remove_S.
    rewrite -> IHl1'. simpl. apply eq_remove_S. symmetry. apply IHl1'.
Qed.
...
Proof.
  ...
  Case "l is x :: l'.". intros n H. 
    simpl in H. destruct n as [| n'].
    SCase "n is O.". inversion H.
    SCase "n is S n'.". apply eq_add_S in H.
    apply IHl' in H. simpl.  rewrite -> app_length_repl. simpl. 
    rewrite -> plus_comm. simpl. apply eq_remove_S.
